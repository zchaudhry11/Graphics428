...
float CompositeBenchmarkTechnique01::_computeTotalBenchmarkScore(SimulationMetricsCollector * simulationMetrics) {
	...
	if (_numAgents == 0.0f)
	{
		std::cout << "exited benchmark --- numAgents = 0" << std::endl;
		return 0.0f; 
	}
	...
	std::cout << "      (alpha, beta, gamma) weights: (" << _alpha << "," << _beta << "," << _gamma << ")\n";
	std::cout << "                      weighted sum: " << _alpha << "*" << _numCollisionsOfAllAgents << " + " << _beta << "*" << agentMetrics->getCurrentMetrics()->totalTimeEnabled << " + " << _gamma << "*" << agentMetrics->getCurrentMetrics()->sumTotalOfInstantaneousKineticEnergies << " = " << _totalBenchmarkScore << "\n";
