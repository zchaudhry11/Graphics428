...
float CompositeBenchmarkTechnique02::_computeTotalBenchmarkScore(SimulationMetricsCollector * simulationMetrics) 
{
	...
	std::cout << "\n";
	std::cout << " number of collisions for the agent: " << _numCollisionsOfAllAgents << "\n";
	std::cout << "      total time spent by the agent: " << agentMetrics->getCurrentMetrics()->totalTimeEnabled << "\n";
	std::cout << "    total energy spent by the agent: " << agentMetrics->getCurrentMetrics()->sumTotalOfInstantaneousKineticEnergies << "\n";
	std::cout << " sum of instantaneous accelerations: " << agentMetrics->getCurrentMetrics()->sumTotalOfInstantaneousAcceleration << "\n";
	std::cout << "(alpha, beta, gamma, delta) weights: " << "(" << _alpha << "," << _beta << "," << _gamma << "," << _delta << ")\n";
	std::cout << "                       weighted sum: " << _alpha << "*" << _numCollisionsOfAllAgents << " + " << _beta << "*" << agentMetrics->getCurrentMetrics()->totalTimeEnabled << " + " << _gamma << "*" << agentMetrics->getCurrentMetrics()->sumTotalOfInstantaneousKineticEnergies << " + " << _delta << "*" << agentMetrics->getCurrentMetrics()->sumTotalOfInstantaneousAcceleration << " = " << _totalBenchmarkScore << "\n";
	std::cout << "                        final score: " << _totalBenchmarkScore << "\n";
